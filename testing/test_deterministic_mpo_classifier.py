...
from deterministic_mpo_classifier import mpo_encoding, class_encode_mps_to_mpo, add_sublist, adding_batches, prepare_batched_classifier
...
def test_class_encode_mps_to_mpo():
    ...
    for label in possible_labels:
        for i, (mpo_site, mps_site, bs_site) in enumerate(
            zip(truncated_mpo_train[label], mps_train[0], truncated_quimb_hairy_bitstrings[label])
        ):
            if i < (n_sites - n_hairysites):
                # same as projecting onto |00> state
                assert np.array_equal(mpo_site[:, 0, :, :], mps_site.data)
    ...
    for label in possible_labels:
        for i, (mpo_site, mps_site, bs_site) in enumerate(
            zip(untruncated_one_site_mpo_train[label], mps_train[0], one_site_quimb_hairy_bitstrings[label])
        ):
            if i < (n_sites - 1):
                # same as projecting onto |00> state
                assert np.array_equal(mpo_site[:, 0, :, :], mps_site.data)
    ...
    truncated_one_site_mpo_train = [
        class_encode_mps_to_mpo(mps_train[0], label, truncated_one_site_quimb_hairy_bitstrings)
        for label in possible_labels
    ]

    for label in possible_labels:
        for i, (mpo_site, mps_site, bs_site) in enumerate(
            zip(truncated_one_site_mpo_train[label], mps_train[0], truncated_one_site_quimb_hairy_bitstrings[label])
        ):
            if i < (n_sites - 1):
                # same as projecting onto |00> state
                assert np.array_equal(mpo_site[:, 0, :, :], mps_site.data)
...
def test_prepare_batched_classifier():
    ...
    multiple_site_prepared_classifier = prepare_batched_classifier(arr_x_train, arr_y_train, D_max, batch_num, one_site = False)

    for k, (prep_site,site) in enumerate(zip(multiple_site_prepared_classifier.data, fMPOs[0].data)):
    ...
    #one site
    one_site_prepared_classifier = prepare_batched_classifier(arr_x_train, arr_y_train, D_max, batch_num, one_site = True)
    one_site_mpo_train = mpo_encoding(mps_train, y_train, truncated_one_site_quimb_hairy_bitstrings)
    fMPOs_one_site = [fMPO([site.data for site in mpo.tensors]) for mpo in one_site_mpo_train]

    for k, (prep_site,site) in enumerate(zip(one_site_prepared_classifier.data, fMPOs_one_site[0].data)):
        d1, s1, i1, j1 = prep_site.shape
        d2, s2, i2, j2 = site.shape
    ...
    classifier_data = multiple_site_prepared_classifier.compress(D=D_max, orthogonalise=False)
    squeezed_multiple_site_mpo_classifier = data_to_QTN(classifier_data.data).squeeze()

    classifier_data = one_site_prepared_classifier.compress_one_site(D=D_max, orthogonalise=False)
    ...
    classifier_data = multiple_site_prepared_classifier.compress(D=D_max, orthogonalise=True)
    ortho_squeezed_multiple_site_mpo_classifier = data_to_QTN(classifier_data.data).squeeze()
